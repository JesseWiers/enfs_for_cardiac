...
class EmptyBI:
    def __call__(self, x: jnp.ndarray, p: jnp.ndarray) -> jnp.ndarray:
        """
        Compute the empty bi-invariant (no symmetry), by concatenating
        x_m and p_i.

        Args:
            x: Input coordinates, shape (B, M, D)
            p: Latent poses, shape (B, N, D)

        Returns:
            Concatenated coordinates, shape (B, M, N, 2D)
        """
        x_exp = x[:, :, None, :]  # shape (B, M, 1, D)
        p_exp = p[:, None, :, :]  # shape (B, 1, N, D)
        return jnp.concatenate([p_exp.repeat(x.shape[1], axis=1), 
                                x_exp.repeat(p.shape[1], axis=2)], axis=-1)
...
class TranslationBI:   
    def __call__(self, x: jnp.ndarray, p: jnp.ndarray) -> jnp.ndarray:
        ...
        # x_spatial = x[..., :-1]  # All dims except last (time)
        # p_spatial = p[..., :-1]  # All dims except last (time)
        # x_time = x[..., -1:]    # Just time dimension
        
        # # Compute relative positions only for spatial dimensions
        # rel_pos_spatial = x_spatial[:, :, None, :] - p_spatial[:, None, :, :]
        
        # # Broadcast time coordinates to match spatial shape
        # time_coords = x_time[:, :, None, :]  # Shape will be (batch, num_coords, 1, 1)
        # time_coords = jnp.broadcast_to(time_coords, rel_pos_spatial.shape[:-1] + (1,))
        
        # # Concatenate spatial relative positions with absolute time
        # return jnp.concatenate([rel_pos_spatial, time_coords], axis=-1)
